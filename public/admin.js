...
function getFilteredCharacters() {
    ...
    const sortMode = String(document.getElementById('char-sort')?.value || 'newest');

    let filtered = characters;
    if (q) {
        filtered = characters.filter(c => [c.id, c.npc_name]
    ...
    return [...filtered].sort((a, b) => {
        if (sortMode === 'oldest') {
            ...
            return String(a.npc_name || '').localeCompare(String(b.npc_name || ''));
        } else {
            const timeA = a.created_at ? new Date(a.created_at).getTime() : 0;
            const timeB = b.created_at ? new Date(b.created_at).getTime() : 0;
            return timeB - timeA;
